...
package a00820997.bookstore.output;
	...
	public static void outputCustomerReport(final ArrayList<Customer> customerData) throws FileNotFoundException {
		...
		if (customerData != null) {
			try {
				...
				output.format(HEADER_FORMATTER, "#", "ID#", "First Name", "Last Name", "Street", "City", "Postal Code",
						"Phone #", "Email", "Join Date", "Length" + "%n");
				output.format(LINE + "%n");
				LOG.info("OUTPUTTING CUSTOMER REPORT TO FILE");
				for (int i = 0; i < customerData.size(); i++) {
					int cIndex = i;
					cIndex += 1;
					output.format(FORMATTER, cIndex, customerData.get(i).getId(), customerData.get(i).getFirstName(),
							customerData.get(i).getLastName(), customerData.get(i).getStreetName(),
							customerData.get(i).getCity(), customerData.get(i).getPostalCode(),
							customerData.get(i).getPhoneNumber(), customerData.get(i).getEmailAddress(),
							customerData.get(i).getJoinDate().format(DATE_FORMATTER),
							generateLength(customerData.get(i).getJoinDate()));
				}
